...
const selectedLink = ref<Map | undefined>();
...
const handleOpenCreateModal = (link: Map) => {
...
const onPriorityChange = async () => {
  try {
    const promises = draggableLinks.value?.map((link: Map, index) => {
      if (link.spec) {
...
async function handleMoveInBatch(group: MapGroup) {
